...
async def file_upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 保存文件到指定的存储目录中
        temp_file_path = os.path.join(storage_dir, f"{formatted_current_time}_{file.filename}")
        ...
        return JSONResponse(content={"code": 200, "message": "文件上传成功", "data": {"file_id": file_id}})
...
async def stt_task(file_id: str = Form(...)):
    try:
        ...
        return JSONResponse(content={"code": 200, "message": "文件正在后台处理", "data": {"task_id": task.id}})
    except Exception as e:
        logger.exception(e)      
